...
class EsatgasHydrantKota extends Model
{

    public $timestamps = false;

    protected $connection = 'esatgas';
    protected $table = 'sarop_hydrant_kota';
    protected $primaryKey = 'id_hydrant_kota';
    ...
    protected $fillable = [
        ...
        'tahun_pengadaan',
        'kondisi_unit',
        'hydrant_kopling_baik',
        'hydrant_kopling_rusak',
        'hydrant_kopling_keterangan',
        'tanda_lampu_hydrant_baik',
        'tanda_lampu_hydrant_rusak',
        'tanda_lampu_hydrant_keterangan',
        'kondisi_air',
        'air_keterangan',
        'kondisi_cat',
        'cat_hydrant_keterangan',
        'patok_pengaman_hydrant_baik',
        'patok_pengaman_hydrant_rusak',
        'patok_pengaman_hydrant_keterangan',
        'meteran_baik',
        'meteran_rusak',
        'meteran_keterangan',
        'no_kota',
    ...
    {
        $path = config('filesystems.assets.esatgas') . "sarana_operasi/hydrant_kota/";
        $query->selectRaw("id_hydrant_kota AS id,kd_hydrant_kota AS `code`");
        $query->selectRaw("CONCAT('" . $path . "', tautan_foto) AS image");
        $query->selectRaw("sarop_hydrant_kota.*");
    ...
    public function getInfosAttribute()
    {
        $attributes[] =
            [
                "type" => "value",
                "name" => "Kopling",
                "baik" => (int)$this->hydrant_kopling_baik,
                "rusak" => (int)$this->hydrant_kopling_rusak,
                "note" => (string)_strip($this->hydrant_kopling_keterangan),
            ];
        $attributes[] =
            [
                "type" => "value",
                "name" => "Tanda Lampu",
                "baik" => (int)$this->tanda_lampu_hydrant_baik,
                "rusak" => (int)$this->tanda_lampu_hydrant_rusak,
                "note" => (string)_strip($this->tanda_lampu_hydrant_keterangan),
            ];
        $attributes[] =
            [
                "type" => "condition",
                ...
                "condition" =>
                [
                    "name" => (string)strtoupper($this->kondisi_air),
                    "label" => (string)($this->kondisi_air == "ada" ? "success" : "danger"),
                ],
                "note" => (string)_strip($this->air_keterangan),
            ];
        $attributes[] =
            [
                "type" => "condition",
                ...
                "note" => (string)_strip($this->cat_hydrant_keterangan),
            ];
        $attributes[] =
            [
                "type" => "value",
                "name" => "Patok Pengaman",
                "baik" => (int)$this->patok_pengaman_hydrant_baik,
                "rusak" => (int)$this->patok_pengaman_hydrant_rusak,
                "note" => (string)_strip($this->patok_pengaman_hydrant_keterangan),
            ];
        $attributes[] =
            [
                "type" => "value",
                "name" => "Meteran",
                "baik" => (int)$this->meteran_baik,
                "rusak" => (int)$this->meteran_rusak,
                "note" => (string)_strip($this->meteran_keterangan),
            ];
        return $attributes;
    }
