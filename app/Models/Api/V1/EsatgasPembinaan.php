...
class EsatgasPembinaan extends Model
{
    ...
    protected $primaryKey = 'no_pembinaan';
    ...
    {
        $path = config('filesystems.assets.esatgas') . "pembinaan/";
        $query->selectRaw("pembinaan.no_pembinaan AS id,pembinaan.kd_pembinaan AS `code`,pembinaan.keterangan AS note");
        $query->selectRaw("CONCAT(DATE_FORMAT(tanggal, '%d %M %Y'),' ', LEFT(jam_awal,5), ' - ', LEFT(jam_akhir,5)) AS `period`");
        $query->selectRaw("CONCAT(tanggal, ' ', jam_awal) AS `start`,CONCAT(tanggal, ' ', jam_akhir) AS `end`");
        $query->selectRaw("CONCAT(FLOOR(TIME_TO_SEC(TIMEDIFF(jam_akhir,jam_awal)) / 60), ' menit') AS `duration`");
        $query->selectRaw("CONCAT('" . $path . "', pembinaan.tautan_foto) AS image");
        $query->selectRaw("pembinaan.*");
        ...
        if (!in_array($user->role_id, [1, 2, 3]) && !$user->bypass_area) {
            if (in_array($user->level->id, [99])) {
                $query->whereRaw("no_pegawai = '" . $user->nrk . "'");
            } else {
                $pos_id = $user->pos->id;
                $where_area = NULL;
    ...
    public function scopeFilter(Builder $query, Request $request)
    {
        $query->when($request->string('search'), function (Builder $query, string $value) {
            ...
            ->when($request->string('period'), function (Builder $query, string $value) use ($request) {
                if ($value) {
                    if ($value == 'today') {
                        $query->whereRaw('DATE(pembinaan.created_at) = CURDATE()');
                    } else if ($value == 'yesterday') {
                        $query->whereRaw('DATE(pembinaan.created_at) = DATE_SUB(CURDATE(), INTERVAL 1 DAY)');
                    } else if ($value == 'lastweek') {
                        $query->whereRaw('DATE(pembinaan.created_at) >= DATE_SUB(CURDATE(), INTERVAL 1 WEEK)');
                    } else if ($value == 'lastmonth') {
                        $query->whereRaw('DATE(pembinaan.created_at) >= DATE_SUB(CURDATE(), INTERVAL 1 MONTH)');
                    } else if ($value == 'daterange' && $request->string('start_date') and $request->string('end_date')) {
                        $query->whereRaw('DATE(pembinaan.created_at) BETWEEN ? AND ?', [$request->string('start_date'), $request->string('end_date')]);
                    }
                }
            })
            ->when($request->string('status'), function (Builder $query, string $value) {
                if ($value) {
                    $query->whereIn('pembinaan.status_pembinaan', explode(',', $value));
                }
            })
            ->when($request->string('sort', 'latest'), function (Builder $query, string $value) {
                if ($value == 'oldest') {
                    $query->orderByRaw('pembinaan.created_at ASC');
                } else {
                    $query->orderByRaw('pembinaan.created_at DESC');
                }
            });
        return $query;
    }
    ...
    {
        return ($this->user_login == $this->no_pegawai && $this->status_pembinaan == 'BELUMVALIDASI');
    }

    public function getIsUpdatableAttribute()
    {
        return ($this->user_login == $this->no_pegawai && $this->status_pembinaan != 'DISETUJUI');
    }

    public function getIsRejectableAttribute()
    {
        return (isset($this->is_verificator) && $this->status_pembinaan == 'BELUMVALIDASI');
    }
