...
class EsatgasSepedaPemadam extends Model
{

    public $timestamps = false;

    protected $connection = 'esatgas';
    protected $table = 'sarop_sepeda_pemadam';
    protected $primaryKey = 'id_sepeda_pemadam';
    ...
    {
        $path = config('filesystems.assets.esatgas') . "sarana_operasi/sepeda_pemadam/";
        $query->selectRaw("id_sepeda_pemadam AS id,kd_sepeda_pemadam AS `code`");
        $query->selectRaw("CONCAT('" . $path . "', tautan_foto) AS image");
        $query->selectRaw("sarop_sepeda_pemadam.*");
