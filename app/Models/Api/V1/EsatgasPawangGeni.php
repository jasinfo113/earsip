...
class EsatgasPawangGeni extends Model
{

    public $timestamps = false;

    protected $connection = 'esatgas';
    protected $table = 'sarop_pawang_geni';
    protected $primaryKey = 'id_pawang_geni';
    ...
    {
        $path = config('filesystems.assets.esatgas') . "sarana_operasi/pawang_geni/";
        $query->selectRaw("id_pawang_geni AS id,kd_pawang_geni AS `code`");
        $query->selectRaw("CONCAT('" . $path . "', tautan_foto) AS image");
        $query->selectRaw("sarop_pawang_geni.*");
    ...
    public function getInfosAttribute()
    {
        $attributes[] =
            [
                "type" => "value",
                "name" => "Pawang Geni",
                "baik" => (int)$this->pawang_geni_baik,
                "rusak" => (int)$this->pawang_geni_rusak,
                "note" => (string)_strip($this->pawang_geni_keterangan),
            ];
        return $attributes;
    }
